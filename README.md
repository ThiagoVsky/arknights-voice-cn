# Arknights operator voice lines, Chinese (Mandarin)
...
The game data refers to this set as `CN_MANDARIN` in the `voiceLangDict` field of `charword_table.json`.

## Updating

`.github/workflows/update.yml` runs once a day. Incremental state lives in
`.state/voice-cn.json`, so only bundles that are new or whose hash changed are downloaded
...
`voice-cn`. The canonical copy lives in [ArknightsGameDataEN](https://github.com/ThiagoVsky/ArknightsGameDataEN) under
